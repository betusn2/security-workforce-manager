// ============================================
// Import tables vers Railway - avec retry
// ============================================
...
async function executeWithRetry(connection, sql, description, maxRetries = 3) {
  for (let i = 0; i < maxRetries; i++) {
    try {
      ...
      if (error.code === 'ER_TABLE_EXISTS_ERROR' || error.message.includes('Duplicate')) {
        console.log(`⚠️  ${description} - Déjà existant`);
        return true;
      }
      if (i === maxRetries - 1) throw error;
      console.log(`⚠️  Retry ${i + 1}/${maxRetries} pour ${description}...`);
      await sleep(2000);
    }
  }
}
...
async function createAllTables() {
  console.log('🚀 Création des tables Railway MySQL...\n');
  ...
  try {
    ...
    await executeWithRetry(connection, `
    ...
    console.log('\n👥 Utilisateurs:');
    userList.forEach(u => {
      console.log(`   ${u.role.toUpperCase()}: ${u.first_name} ${u.last_name} (${u.employee_id})`);
    });
